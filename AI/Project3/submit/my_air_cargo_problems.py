...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def load_actions():
            ...
            '''
            loads = []
            L = [ (c,p,a) for c in self.cargos for p in self.planes for a in self.airports ]
            for x in L:
                C,P,A = x
    ...
    def result(self, state: str, action: Action):
        ...
        new_state = FluentState(kb.clauses,neg)
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        for act in self.actions_list:
            #for clause in act.effect_rem:
            #    kb.retract(act.substitute(clause, args))
            # add positive literals
            for clause in act.effect_add:
                if clause in self.goal and not clause in kb.clauses:
                    count += 1
                    kb.tell(act.substitute(clause, act.args))
