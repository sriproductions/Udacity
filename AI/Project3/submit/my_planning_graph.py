...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            ...
            for nd in newnode.prenodes:
                if not nd in self.s_levels[level]:
                    break
                nd.children.add(newnode)
                newnode.parents.add(nd)
    ...
    def add_literal_level(self, level):
        ...
        for anode in self.a_levels[level-1]:
            for snode in anode.effnodes:
                newnode = PgNode_s(snode.symbol, snode.is_pos)
                anode.children.add(newnode)
                newnode.parents.add(anode)
                self.s_levels[level].add(newnode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        for literal in node_a1.action.effect_add:
            if literal in node_a2.action.effect_rem:
                return True
        for literal in node_a1.action.effect_rem:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        if node_a2.is_persistent:
            for literal in node_a1.action.effect_add:
                if literal in node_a2.action.precond_neg:
                    return True
            for literal in node_a1.action.effect_rem:
                if literal in node_a2.action.precond_pos:
                    return True

        if node_a1.is_persistent:
            for literal in node_a2.action.effect_add:
                if literal in node_a1.action.precond_neg:
                    return True
            for literal in node_a2.action.effect_rem:
                if literal in node_a1.action.precond_pos:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        if type(node_a1) != type(node_a2):
            return False

        for p1 in node_a1.parents:
            for p2 in node_a2.parents:
                for i in p1.mutex:
                    if p2.literal == i.literal:
                        return True
                for i in p2.mutex:
                    if p1.literal == i.literal:
                        return True
        return False
